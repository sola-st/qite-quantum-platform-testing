...
class DDMin:
    def __init__(
            self, circumstances, test_func, default_partition=2,
            kwargs_test_func=None):
        self.circumstances = circumstances
        self.test = test_func
        self.part = default_partition
        if kwargs_test_func is None:
            kwargs_test_func = {}
        self.kwargs_test_func = kwargs_test_func
    ...
    def execute(self):
        ...
        # Hard condition, empty must not trigger
        assert self.test(
            [], **self.kwargs_test_func) == True, f"We expected the empty program to return True: {self.test([], **self.kwargs_test_func)}"
        # The circumstances must trigger
        assert self.test(
            self.circumstances, **self.kwargs_test_func) == False, f"We expected the exact buggy program to return False (namely the same error): {self.test(self.circumstances, **self.kwargs_test_func)}"

        # Usually start with binary, 2, but...
        partition = self.part
        ...
        while len(self.circumstances) >= 2:
            ...
            for subset in subsets:
                ...
                if self.test(complement, **self.kwargs_test_func) == False:
